...
//$mail -> AddCC ('satria.hendra@daihatsu.astra.co.id', 'Hanif Rahardiyan'); //email cc3
		...
		$lisen_eng	     = "SELECT * FROM lcs_lisensi WHERE exprd_lcns <= now() AND plan LIKE 'GA-EHS' ";
		$query_lisen_eng = mysql_query($lisen_eng);
		$count_lisen_eng = mysql_num_rows($query_lisen_eng);
		$lisen_eng_war   		= "SELECT * FROM lcs_lisensi WHERE tgl_war <= now() AND plan LIKE 'GA-EHS' ";
		...
		$lisen_eng_ok	= ($count_lisen_gaehs - ($count_lisen_eng + $eng_war));
		
		$laporan	= "<html>";
		$laporan   .= "
				<table border=1 cellspacing=0 cellpadding=0 width=500>
					<tr align=center bgcolor='#3399FF'>
						<th colspan=3><font color=white>List License GA-EHS - General Affairs-EHS</font></th>
					</tr>
					<tr align=center>
					...
					<tr align=center>
						<td colspan=3><b>$count_lisen_gaehs Licenses</b></td>
					</tr>
				</table>
		";
		...
		$no=0;
		$cariexpired=mysql_query("SELECT ket_lcns,no_lcns,exprd_lcns, DATEDIFF(exprd_lcns,now()) as remain2 FROM lcs_lisensi WHERE exprd_lcns <= now() AND plan LIKE 'GA-EHS' ");
		...
		<tr bgcolor=black><td><font color=white>NO</font></td><td><font color=white>ITEM</font></td><td><font color=white>NO IJIN</font></td><td><font color=white>EXP DATE</font></td><td><font color=white>STATUS</font></td></tr>";
		$no=0;
		$cariwar=mysql_query("SELECT ket_lcns,no_lcns,exprd_lcns, DATEDIFF(exprd_lcns,now()) as remain FROM lcs_lisensi WHERE  plan LIKE 'GA-EHS'
		...
		$message2				= "Berikut daftar lisensi Warning dan Expired untuk General Affairs-EHS section.";
		$body					= $laporan;
