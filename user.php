                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header" style="background: green; color: white;">
                                <strong class="card-title">List All Users LCS</strong>
                            </div>
                            <div class="card-body">
                                <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <?php
													$query = mysql_query ("select * from lcs_login", $con);
													// cek total row
													$total_row = mysql_num_rows($query);
													if ( $total_row > 0 ) {
														// kalau lebih dari 0, berarti ambil row
														$no = 1;
														$lihat = mysql_query("select * from lcs_login");
														while ( $row = mysql_fetch_array($lihat) ) {
															...
															echo '<a href="user_edit.php?id='.$row['id'].'"class="text-secondary"><i class="fa fa-edit"></i></a> &nbsp; &nbsp;|&nbsp; &nbsp; ';
															echo '<a href="del_user.php?id='.$row['id'].'"class="text-secondary"><i class="fa fa-trash-o" onclick="return confirm(\'Apakah Anda yakin ingin menghapus user atas nama '.$row['nama'].' ?\')"></i></a>';
															echo '</td>';
															echo '</tr>';
														}
													} else { // kalau row = 0, beri keterangan tidak ada data
														echo '<tr align=center><td colspan="8"><h5>User Tidak Tersedia.</h5></td></tr>';
													}
										?>
                                    </tbody>
                                </table>
								<a href="dnld_lcns.php"><button type="button" class="btn btn-success"><i class="fa fa-windows"></i>&nbsp; Download to Excel</button></a>
                            </div>
                        </div>
                    </div>
                </div>
